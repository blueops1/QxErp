...
begin   //proc_insert_cpcrkmxz_cprk   cpbh,rksl,jzdate,memo
  ...
  try
    ...
    begin
      close;
      StoredProcName:='proc_insert_bcpcrkmxz_rk'; //bcpbh,rksl,jzdate,czry
      ...
      begin
        ...
        combobox1.SetFocus;
        application.MessageBox('保存数据成功！','半成品入库提示');
    end;
  except
    application.MessageBox('保存数据失败！','半成品入库提示');
  end;
end;
...
begin
  ...
  if booldouble=true then
    ...
    if (edit2.Text<>'') and (combobox1.Text<>'') then
      ...
      application.MessageBox('请将明细填写完整！','半成品入库提示');
end;
...
procedure TForm31.StringGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  tmppoint:tpoint;
begin
...
procedure TForm31.StringGrid1SelectCell(Sender: TObject; ACol, ARow: Integer;
  var CanSelect: Boolean);
begin
  selRowIndex:=ARow;
end;
